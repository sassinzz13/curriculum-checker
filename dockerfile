...
RUN apt-get update && \
    apt-get install -y curl gnupg2 unixodbc-dev libgssapi-krb5-2 && \
...
RUN odbcinst -q -d -n "ODBC Driver 18 for SQL Server" || echo "ODBC Driver 18 not found"

# List installed ODBC drivers for diagnostic purposes
RUN odbcinst -q -d
